...
make_hho_cut_stabilization_extended(const cuthho_mesh<T, ET>& msh,
    ...
    // size_t local_dofs_ex = local_dofs; 
    ...
    //     local_dofs_ex += cbs;
    // }
    // size_t extended_dofs = dp_cells.size()*local_dofs_ex;
    // // Loop over dependent cells 
    // size_t offset_dofs = 2*local_dofs; // cpt 
    // for (auto &dp_cl : dp_cells) {
    //     auto dp_cell = msh.cells[dp_cl];
    //     fcs = faces(msh, dp_cell);
    //     // Loop over dependent faces
    //     for (size_t i = 0; i < fcs.size(); i++) {
    //         auto fc = fcs[i];
    // }
...
template<typename T, size_t ET, typename Function>
Matrix<typename cuthho_mesh<T, ET>::coordinate_type, Dynamic, Dynamic>
make_hho_stabilization_interface_TOK(const cuthho_mesh<T, ET>& msh,
                                 const typename cuthho_mesh<T, ET>::cell_type& cl,
                                 const Function& level_set_function,
                                 const hho_degree_info& di,
                                 const params<T>& parms = params<T>(), bool scaled_Q = true) {
    ...
    ////////////////////////////// Stabilization
    const auto stab_n = make_hho_cut_stabilization_extended(msh, cl, di,element_location::IN_NEGATIVE_SIDE, scaled_Q);
    const auto stab_p = make_hho_cut_stabilization_extended(msh, cl, di,element_location::IN_POSITIVE_SIDE, scaled_Q);
    ...
    const auto stab_n_ex = make_hho_cut_stabilization_extended(msh, cl, di,element_location::IN_NEGATIVE_SIDE, scaled_Q);
    const auto stab_p_ex = make_hho_cut_stabilization_extended(msh, cl, di,element_location::IN_POSITIVE_SIDE, scaled_Q);
    // cells--cells
    data.block(0, 0, cbs, cbs)                   += parms.kappa_1 * stab_n.block(0, 0, cbs, cbs);
    data.block(local_dofs, local_dofs, cbs, cbs) += parms.kappa_2 * stab_p.block(0, 0, cbs, cbs);
    // cells--faces
...
template<typename T, size_t ET, typename Function>
Matrix<typename cuthho_mesh<T, ET>::coordinate_type, Dynamic, Dynamic>
make_hho_stabilization_interface_TKO_NEG(const cuthho_mesh<T, ET>& msh,
                                 const typename cuthho_mesh<T, ET>::cell_type& cl,
                                 const Function& level_set_function,
                                 const hho_degree_info& di,
                                 const params<T>& parms = params<T>(), bool scaled_Q = true) {
    ...
    // data.block(cbs, cbs, cbs, cbs) += parms.kappa_2 * stab_p.block(0, 0, cbs, cbs);
    // // cells--faces
    // data.block(cbs, 2*cbs, cbs, num_faces*fbs) += parms.kappa_2 * stab_p.block(0, cbs, cbs, num_faces*fbs);
    // // faces--cells
    // data.block(2*cbs, cbs, num_faces*fbs, cbs) += parms.kappa_2 * stab_p.block(cbs, 0, num_faces*fbs, cbs);
    // // faces--faces
    // data.block(2*cbs, 2*cbs, num_faces*fbs, num_faces*fbs) += parms.kappa_2 * stab_p.block(cbs, cbs, num_faces*fbs, num_faces*fbs);
...
template<typename T, size_t ET, typename Function>
Matrix<typename cuthho_mesh<T, ET>::coordinate_type, Dynamic, Dynamic>
make_hho_stabilization_interface_TKO_POS(const cuthho_mesh<T, ET>& msh,
                                 const typename cuthho_mesh<T, ET>::cell_type& cl,
                                 const Function& level_set_function,
                                 const hho_degree_info& di,
                                 const params<T>& parms = params<T>(), bool scaled_Q = true) {
    ...
    const auto num_faces = faces_extended_TKOibar(msh, cl, element_location::IN_NEGATIVE_SIDE).first.size();

    Matrix<T, Dynamic, Dynamic> data = Matrix<T, Dynamic, Dynamic>::Zero(2*cbs+num_faces*fbs, 2*cbs+num_faces*fbs);
    cell_basis<cuthho_mesh<T, ET>,T> cb(msh, cl, celdeg);
    auto hT = diameter(msh, cl);

    // Matrix<T, Dynamic, Dynamic> stab_n = make_hho_cut_stabilization_TKOibar(msh, cl, di, element_location::IN_NEGATIVE_SIDE, scaled_Q);
    // Matrix<T, Dynamic, Dynamic> stab_p = make_hho_cut_stabilization_TKOi(msh, cl, di, element_location::IN_POSITIVE_SIDE, scaled_Q);    
...
make_hho_gradrec_vector_extended(const cuthho_mesh<T, ET>& msh, const typename cuthho_mesh<T, ET>::cell_type& cl, const hho_degree_info& di, const Function& level_set_function) {
    ...
    if (celdeg > 0) {
        ...
        for (auto& qp : qps) {
            ...
            gr_rhs.block(0, 0, gbs, cbs) += qp.second * g_phi * c_dphi.transpose(); // Cell term
        }
    }
...
make_hho_gradrec_vector_interface_TOK(const cuthho_mesh<T, ET>& msh,
    ...
    if (where == element_location::IN_NEGATIVE_SIDE)
        gr_rhs.block(0, 0, gbs, local_dofs) += rhs_tmp.block(0, 0, gbs, local_dofs);
    if (where == element_location::IN_POSITIVE_SIDE) 
        gr_rhs.block(0, local_dofs, gbs, local_dofs) += rhs_tmp.block(0, 0, gbs, local_dofs);
    ...
    size_t extended_dofs = dp_cells.size()*local_dofs_ex;
    rhs_tmp = matrix_type::Zero(gbs, extended_dofs);
    // std::cout << "LOCAL DOFS = " << 2*local_dofs << std::endl;
    // std::cout << "EXTENDED DOFS = " << extended_dofs << std::endl;
    ...
    for (auto &dp_cl : dp_cells) {
        ...
        if (dp_cell.user_data.location == element_location::IN_NEGATIVE_SIDE) {
            ...
            // neg terms of the current cell
            gr_rhs.block(0, 0, gbs, cbs) += interface_term.block(0, 0, gbs, cbs);        
            // pos terms the extended term                  
            gr_rhs.block(0, offset_dofs + local_dofs, gbs, cbs) += interface_term.block(0, cbs, gbs, cbs); 
        }
    // std::cout << "INTERFACE TERM OK" << std::endl;
    ...
    // std::cout << "EXTENDED CELL OK" << std::endl;
        // Extended face term
        fcs = faces(msh, dp_cell);
        ns  = normals(msh, dp_cell);
        num_faces = fcs.size();
        for (size_t i=0; i < num_faces; i++) {
            ...
            for (auto& qp : qps_f) {
                ...
                const vector_type qp_g_phi_n = qp.second * g_phi * n;
                // Extended face terms
                rhs_tmp.block(0, offset_dofs-2*local_dofs + cbs + i*fbs, gbs, fbs) += qp_g_phi_n * f_phi.transpose(); 
                // Current cell terms
                rhs_tmp.block(0, 0, gbs, cbs) -= qp_g_phi_n * c_phi.transpose();                         
            }
        }
    ...
    // std::cout << "EXTENDED FACE OK" << std::endl;
    if (where == element_location::IN_NEGATIVE_SIDE) {
        gr_rhs.block(0, 2*local_dofs, gbs, extended_dofs) += rhs_tmp.block(0, 0, gbs, extended_dofs);
    }
    if (where == element_location::IN_POSITIVE_SIDE) {
        auto nb_dp_cell_neg = cl.user_data.dependent_cells_neg.size();
        gr_rhs.block(0, 2*local_dofs+nb_dp_cell_neg*(local_dofs+cbs), gbs, extended_dofs) += rhs_tmp.block(0, 0, gbs, extended_dofs);
    }
...
//     // New size
//     // matrix_type oper = matrix_type::Zero(gbs, 2*cbs + num_faces_TKOibar*fbs);
//     // matrix_type data = matrix_type::Zero(2*cbs + num_faces_TKOibar*fbs, 2*cbs + num_faces_TKOibar*fbs);
...
make_hho_gradrec_vector_interface_TKOibar(const cuthho_mesh<T, ET>& msh,
    ...
    ////////////////////////////////////////////////// INTERFACE TERM CURRENT CELL
    if (where == element_location::IN_NEGATIVE_SIDE) {
        std::cout << "(TKO) TERMES D'INTERFACES A MODIFIER" << std::endl;
    }
    ...
    size_t extended_dofs = dp_cells.size()*local_dofs;
    rhs_tmp = matrix_type::Zero(gbs, extended_dofs);
    auto cpt = 0; // cpt of dependent cells
    ...
    for (auto &dp_cl : dp_cells) {
        ...
        for (auto& qp : qps) {
            const auto c_dphi = cb.eval_gradients(qp.first);
            const auto g_phi  = gb.eval_basis(qp.first);
            rhs_tmp.block(0, cpt*local_dofs, gbs, cbs) += qp.second * g_phi * c_dphi.transpose(); 
        }
        ...
        // Interface term of the extended cells
        if (where == element_location::IN_NEGATIVE_SIDE) {
            std::cout << "(TKOK) AJOUTER LES TERMES D'INTERFACE POUR LES CELLULES DEPENDANTES: " << offset(msh, msh.cells[dp_cl]) << std::endl;
        }
        cpt++;
    }
    if (where == element_location::IN_NEGATIVE_SIDE) {
        gr_rhs.block(0, local_dofs, gbs, extended_dofs) += rhs_tmp.block(0, 0, gbs, extended_dofs);
    }
    if (where == element_location::IN_POSITIVE_SIDE) {
        auto nb_dp_cell_neg = cl.user_data.dependent_cells_neg.size();
        gr_rhs.block(0, local_dofs+nb_dp_cell_neg*local_dofs, gbs, extended_dofs) += rhs_tmp.block(0, 0, gbs, extended_dofs);
    }
